...
METADATA
{
    ...
    "description": {
        "zh": "随机连三链系统：空闲态每15分钟查日志判断用户是否沉默，运行态每分钟x累加、按公式算y(概率)、掷骰子命中则主动发消息。支持面板实时查看xy状态、手动触发、公式编辑与冷却配置。内置日历：每天12点后联网拉取节假日，可点选全天静默日或开启节假日自动静默。",
        "en": "Random triple-chain system: idle checks logs every 15min, running increments x each minute, computes y via formula, rolls dice to call owner proactively. Built-in calendar with holiday auto-fetch after 12:00 daily and per-day full-day quiet."
    },
...
async function fetchHolidayYear(year) {
    try {
        ...
        var req = client.newRequest()
            ...
            .method("GET")
            .headers({ "User-Agent": "Operit/random_chain" })
            .build();
...
async function getRecentChatContext(formula) {
    try {
        if (Tools && Tools.Chat && typeof Tools.Chat.getMessages === "function") {
            var msgs = await fetchChatMessages(formula.chat_id, 12);
            if (msgs && msgs.length) {
                // 取正文文本，简单拼成对话上下文
                var lines = [];
...
exports.api_docs = async function (params) {
    var docs = {
        ...
        tools: [
            ...
            { name: "coax", zh: "安抚/重置计数：复位x、未命中与嫉妒计数并联动温柔巡检回落", params: [], returns: "data: {x, miss_count, jealousy_count, jealousy_stopped, gentle_jealousy}", example: "random_chain:coax" },
            ...
            { name: "update_formula", zh: "更新公式与配置参数", params: [{ name: "a/b/c", type: "number", required: false }, { name: "idle_threshold_minutes", type: "number", required: false }, { name: "cooldown_minutes", type: "number", required: false }, { name: "awake_messages", type: "string(json数组)", required: false }, { name: "send_mode", type: "string(A1/A2)", required: false }, { name: "awake_mode", type: "string(default/custom)", required: false }, { name: "max_wake_stops", type: "number", required: false }, { name: "character_card_name", type: "string", required: false }, { name: "quiet_enabled", type: "boolean", required: false, desc: "静默总开关，true=免打扰" }, { name: "quiet_day_start", type: "string(HH:MM)", required: false, desc: "白天段开始，如 09:00" }, { name: "quiet_day_end", type: "string(HH:MM)", required: false, desc: "白天段结束，如 18:00" }, { name: "quiet_night_start", type: "string(HH:MM)", required: false, desc: "夜间段开始，如 22:00" }, { name: "quiet_night_end", type: "string(HH:MM)", required: false, desc: "夜间段结束，如 09:00，支持跨天" }], returns: "data: {formula}", example: "random_chain:update_formula {a:0.007,b:0.0000006,c:3.15}" },
            { name: "get_formula", zh: "读取当前公式与配置", params: [], returns: "data: {a, b, c, idle_threshold_minutes, cooldown_minutes, send_mode, awake_mode, max_wake_stops, character_card_name, quiet_enabled, quiet_day_start, quiet_day_end, quiet_night_start, quiet_night_end, quiet_suggest}", example: "random_chain:get_formula" },
             { name: "suggest_quiet", zh: "AI/本地算法结合时间与用户习惯总结静默时间建议（白天段+夜间段，可显示、可一键应用）。可传 chat_name（对话名字）或 chat_id 指定要分析的聊天记录，只读消息时间不读内容", params: [{ name: "apply", type: "boolean", required: false, desc: "true=直接应用建议并开启静默，默认false仅生成建议" }, { name: "chat_name", type: "string", required: false, desc: "对话名字，按名字反查对话（优先于 chat_id）" }, { name: "chat_id", type: "string", required: false, desc: "要分析的聊天记录所在对话ID（没传名字时用）" }], returns: "data: {suggestion{day_start,day_end,night_start,night_end,reason,source,generated_at,sample_count,chat_id}, applied, source, chat_id, hour_stats[], peak_hour, sample_count}", example: "random_chain:suggest_quiet {chat_name:'猫娘', apply:true}" },
            { name: "link_agent", zh: "按对话名字反查对话ID并保存为目标对话（不发送任何消息）", params: [{ name: "chat_name", type: "string", required: true, desc: "对话的名字（如：猫娘）" }], returns: "data: {chat_id, title}", example: "random_chain:link_agent {chat_name:'猫娘'}" },
